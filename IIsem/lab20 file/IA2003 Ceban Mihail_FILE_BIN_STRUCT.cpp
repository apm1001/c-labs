...
void binarCoding(struct mebel x[],int n){
...
void getStuct(struct mebel x[],int n){
    f = fopen("struct.dat","rb");
    for(int i=0;i<n;i++){
        fread(&x[i],sizeof(x[i]),1,f);
    }
    fclose(f);
}
        ...
        if(k==1){
            for(int i=0;i<e-1;i++){
                for(int j=0;j<e-1-i;j++){
                    if(b[j].type.price<b[j+1].type.price){
                        temp = b[j];
                        b[j] = b[j+1];
                        b[j+1] = temp;
                    }
            ...
            for(int i=0;i<n-1;i++){
                for(int j=0;j<n-1-i;j++){
                    if(a[j].type.price<a[j+1].type.price){
                        temp = a[j];
                        a[j] = a[j+1];
                        a[j+1] = temp;
                    }
